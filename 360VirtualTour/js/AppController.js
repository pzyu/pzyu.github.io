...
function ShowEditingTourPage(){
	$("#creating-tour").addClass("box-hidden");
    $("#creating-tour").removeClass("box-visible");

    $("#editing-tour-container").addClass("box-visible");
    $("#editing-tour-container").removeClass("box-hidden");
	
	IsShowDefaultViewBtn();
}

function HideEditingTourPage(){
	$("#editing-tour-container").addClass("box-hidden");
    $("#editing-tour-container").removeClass("box-visible");

    $("#creating-tour").addClass("box-visible");
    $("#creating-tour").removeClass("box-hidden");
}

// Show 360 photos container
function ShowAddSceneContainer() {
	ShowUserPhotos();
    $("#media-photo-container").removeClass("hidden");
    $("#media-photo-container").addClass("visible");
}

// Hide 360 photos container
function HideAddSceneContainer() {
    $("#media-photo-container").removeClass("visible");
    $("#media-photo-container").addClass("hidden");
}

//decide to show and hide default view button
function IsShowDefaultViewBtn(){
	var arrySence = $("#scene-scroll-container").find("div.scene-item-container");
	if(arrySence.length > 0){
		$("#default-veiw-container").show();
		$("#default-view-btn").show();
		$("#main-scene").show();
		return true;
	}
	else{
		$("#default-veiw-container").hide();
		$("#default-view-btn").hide();
		$("#main-scene").hide();
		return false;
	}
	return false;
...
function AddScene() {
	var fileInfo = [];
	var itemHtml;
//	g_arrayScene = GetUserScenesFromServer();
	
	for(let i = 0; i < g_arrayScene.length; i++){
		var imageUrl, strName;
		
		if(IsCheckedMedia(i) == false)
			continue;
		
		fileInfo = g_arrayScene[i];
		imageUrl = fileInfo["path"];
		strName = "scene-" + (i+1).toString();
		
		itemHtml = `<div class="scene-item-container" id="scene-item-container-`+ i +`">
						<div class="scene-btn-container">
							<button class="scene-logo-btn" onclick="OnSceneSelect(`+ i +`);">
								<img class="img-fluid scene-logo-img" src="`+ imageUrl +`">
							</button>
							<button class="edit-btn" onclick="OnEditSceneTitle(`+ i +`);"></button>
							<button class="delete-btn" onclick="OnDeleteScene(`+ i +`);"></button>
						</div>
	...
	if(IsShowDefaultViewBtn())
		OnSceneSelect(0);
}

function OnSceneSelect(i){
	var currentSceneImg = $("#scene-item-container-" + i).find("img.scene-logo-img");
	
	if(g_currentScene != -1){
		$("#scene-item-container-" + g_currentScene).find("img.scene-logo-img").css("border", "0px");
	}
	
	g_currentScene = i;
	currentSceneImg.css("border", "4px solid #FBB03B");
	
	LoadProject('sample' + i);
	
	DestroyInteractivesAfterTime();

    // Load scene
    LoadScene(scenes[g_currentScene]);
}

function OnEditSceneTitle(i){
	var titleField = $("#scene-item-container-" + i).find("input.scene-titleField");
	var editBtn = $("#scene-item-container-" + i).find("button.edit-btn");
	
	if(titleField.is('[readonly]')){
		titleField.removeAttr('readonly');
		editBtn.css('background-image', 'url("../images/assets/edit-red.png")');
	}
	else{
		titleField.attr('readonly','readonly');
		editBtn.css('background-image', 'url("../images/assets/edit.png")');
	}
}

function OnDeleteScene(i){
	$("#scene-item-container-" + i).remove();
	IsShowDefaultViewBtn();
}

function GetUserScenesFromServer(){
	var sceneUrls = ["./images/backgrounds/conference room_00000.jpg", "./images/backgrounds/education_00000.jpg", "./images/backgrounds/entrance 2_00000.jpg", "./images/backgrounds/entrance_00000.jpg", "./images/backgrounds/industry 4_00000.jpg", "./images/backgrounds/main area_00000.jpg", "./images/backgrounds/retail_00000.jpg", "./images/backgrounds/room_00000.jpg", "./images/backgrounds/Viewpoint C 2310.png", "./images/backgrounds/Viewpoint E 2310.png"];
...
function ClearListPhotos(){
	//clear media items in media gallery
	$("#media-grid-container").empty();
	
	//init media menu tabs
	$(".media-menu-img").show();
	$(".selected-image").hide();
	$(".media-menu-txt").css("color", "#666666");
}
...
function ShowUserPhotos(){
	...
	$("#sel-open-file-img").show();
	$("#open-file-img").hide();
	$("#open-file-txt").css("color", "#FBB03B");

	//arrayPhoto = GetUserPhotosFromServer();
	arrayPhoto = g_arrayScene;
	...
	for(let i = 0; i < arrayPhoto.length; i++){
		...
		itemHtml = `<div class="col-md-4 media-logo" id="media-logo-container-`+ i +`">
						<button class="media-logo-btn" onclick="OnMediaSelect(`+ i +`);" >
							<img class="img-fluid media-logo-img" src="` + imageUrl + `" alt="">
						</button>
						<h5 class="media-menu-txt">` + strName + `</h5>
						<img class="check-img hidden" src="images/assets/check.png">
					</div>`;
		$("#media-grid-container").append(itemHtml);
	}
}

function ShowTeamPhotos(){
	ClearListPhotos();
	
	$("#sel-team-img").show();
	$("#team-img").hide();
	$("#team-txt").css("color", "#FBB03B");
}

function ShowMusketeerPhotos(){
	ClearListPhotos();
	
	$("#sel-musketeers-img").show();
	$("#musketeers-img").hide();
	$("#musketeers-txt").css("color", "#FBB03B");
}

function IsCheckedMedia(i){
	var checkImg;
	checkImg = $("#media-logo-container-" + i).find("img.check-img");
...
function OnMediaSelect(i){
	...
	mediaLogImg = $("#media-logo-container-" + i).find("img.media-logo-img");
	checkImg = $("#media-logo-container-" + i).find("img.check-img");
...
function OnSetDefaultSceneView(){
	LoadScene(scenes[g_currentScene]);
}
...
function ReadSceneinfo(){
	...
	for(let i = 0; i < g_arrayScene.length; i++){
		...
		response = response + `
					{
   				      "id":"scene-`+ i +`",
   				       "display-name":"scene-`+ i +`",
   				      "background-url":"`+ fileInfo["path"] + `",
   				      "rotation":"0 260 0",
   				      "interactives":[
